...
class myConfigGUI(ThemedTk):
    ...
    def testConn(self):
        ...
        if self.checkTextFields():
            ...
            try:
                msg = requests.get("http://" + RepetierServerIP + ":" + RepetierServerPort + "/printer/info")
                msg = msg.json()
            except:
                msg = "Error"
            if msg == "Error":
                ...
                for printer in msg['printers']:
                    infoMsg += _("      - Printer: %s - found") % printer['name'] + "\n"
                self.rsAPI.text['state'] = "normal"
                self.rsAPI.text.delete(0, tk.END)
                self.rsAPI.text.insert(0,msg['apikey'])
                self.rsAPI.text['state'] = "disabled"
    ...
    def setConfigFile(self):
        configItems = self.configFile['server'] 
        if configItems['LANGUAGE'] != self.language.languageSelect.get():
            logger.info(_("setConfigFile: Changed language from: %s to: %s") % (configItems['LANGUAGE'],self.language.languageSelect.get()))
            configItems['LANGUAGE'] = self.language.languageSelect.get()
        if configItems['RepetierServerIP'] != self.rsNetwork.IP.get():
            logger.info(_("setConfigFile: Changed Server IP from: %s to: %s") % (configItems['RepetierServerIP'],self.rsNetwork.IP.get()))
            configItems['RepetierServerIP'] = self.rsNetwork.IP.get()
        if configItems['RepetierServerPort'] != self.rsNetwork.port.get():
            logger.info(_("setConfigFile: Changed Server Port from: %s to: %s") % (configItems['RepetierServerPort'],self.rsNetwork.port.get()))
            configItems['RepetierServerPort'] = self.rsNetwork.port.get()
        if configItems['MY_REPETIER_SERVER_API_KEY'] != self.rsAPI.text.get():
            logger.info(_("setConfigFile: Changed Repetier API from: %s to: %s") % (configItems['MY_REPETIER_SERVER_API_KEY'],self.rsAPI.text.get()))
            configItems['MY_REPETIER_SERVER_API_KEY'] = self.rsAPI.text.get()
        if configItems['MY_TELEGRAM_TOKEN'] != self.telegramToken.text.get():
            logger.info(_("setConfigFile: Changed Telegram Token from: %s to: %s") % (configItems['MY_TELEGRAM_TOKEN'],self.telegramToken.text.get()))
            configItems['MY_TELEGRAM_TOKEN'] = self.telegramToken.text.get()
        if configItems['MY_TELEGRAM_ID'] != self.telegramID.text.get():
            logger.info(_("setConfigFile: Changed Telegram ID from: %s to: %s") % (configItems['MY_TELEGRAM_ID'],self.telegramID.text.get()))
            configItems['MY_TELEGRAM_ID'] = self.telegramID.text.get()
...
class botLanguageFrame(tk.Frame):
    ...
    def langSelect(self,event):
        self.parent.parent.configFile['server']['LANGUAGE'] = self.languageSelect.get()
